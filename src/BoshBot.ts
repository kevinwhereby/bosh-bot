...
export class BoshBot extends EventEmitter<{ stopped: [null] }> {
  private assistant: Assistant;

  constructor({ assistantKey }: { assistantKey: string }) {
    super();
  ...
  async start({ roomUrl }: { roomUrl: string }) {
    await this.assistant.joinRoom(roomUrl);

    const roomConnection = this.assistant.getRoomConnection();

    const { audioSource: sendAudioSource } =
      await this.assistant.startLocalMedia({
        audio: true,
        video: false,
      });
    if (!sendAudioSource) {
